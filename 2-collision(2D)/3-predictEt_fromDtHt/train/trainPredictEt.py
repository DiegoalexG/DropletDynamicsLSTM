...
def main():
    ...
    entrada = arq["TS"][:, :, :2]
    saida = arq["TS"][:, :, 2:]
    saida = np.array([saida[i] / (saida[i, 0, 0] + saida[i, 0, 1] + saida[i, 0, 2]) for i in range(len(saida))])
    ...
    mcp_save = ModelCheckpoint('predictEt.keras', save_best_only=True, monitor='loss', mode='min')
